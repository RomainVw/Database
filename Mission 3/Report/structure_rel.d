...
OPERATOR DATE_CMP (START ENHANCEDDATE,ENDDATE ENHANCEDDATE) RETURNS INTEGER;
IF THE_YEAR(START) > THE_YEAR(ENDDATE) THEN RETURN 1;
ELSE IF THE_YEAR(START) < THE_YEAR(ENDDATE) THEN RETURN -1;
ELSE IF THE_MONTH(START) > THE_MONTH(ENDDATE) THEN RETURN 1;
ELSE IF THE_MONTH(START) < THE_MONTH(ENDDATE) THEN RETURN -1;
ELSE IF THE_DAY(START) > THE_DAY(ENDDATE) THEN RETURN 1;
ELSE IF THE_DAY(START) < THE_DAY(ENDDATE) THEN RETURN -1;
ELSE RETURN 0;
END IF;
END IF;
...
CONSTRAINT C36 DATE_CMP (RANGERELATION {START}, RANGERELATION {ENDDATE}) <= 0;
CONSTRAINT C37 DATE_CMP (EVENT {BEGINNING}, EVENT {ENDDATE}) <= 0;




CONSTRAINT C36 DATE_CMP(ENHANCEDDATE(2014,4,4), ENHANCEDDATE(2015,4,4)) <= 0;

CONSTRAINT C37 DATE_CMP(RANGERELATION{START}, RANGERELATION{ENDDATE}) <= 0;